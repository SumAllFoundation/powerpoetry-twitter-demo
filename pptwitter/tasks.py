...
from pptwitter.app import api, app as flask
...
ranker = PercentilePoetryRanker(
    os.path.join(basedir, "..", flask.config["INQUIRER_PATH"]),
    os.path.join(basedir, "..", flask.config["COCA_PATH"]),
    os.path.join(basedir, "..", flask.config["PERCENTILES_PATH"]))


@periodic_task(run_every=crontab(minute="*/1"))
def process_poems():
    logger.info("Processing poems...")
    q = flask.config.get("QUERY")
    count = flask.config.get("COUNT")
